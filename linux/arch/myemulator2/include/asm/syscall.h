...
static inline int syscall_get_nr(struct task_struct *task, struct pt_regs *regs) { return regs->orig_r1; }
static inline void syscall_rollback(struct task_struct *task, struct pt_regs *regs) { regs->r[1] = regs->orig_r1; }
