...
test -S "$tmp/qmp.sock"
printf '%s\n' \
  '{"command":"b","args":{"address":260}}' \
  '{"command":"continue"}' |
  "$root/tools/mydebug" --machine --qmp "$tmp/qmp.sock" |
  python3 -c 'import json,sys; r=[json.loads(x) for x in sys.stdin if x.strip()]; assert r[1]["registers"]["pc"] == 260; assert r[1]["registers"]["r0"] == 42'
echo 'MyEmulator floppy boot test passed'
