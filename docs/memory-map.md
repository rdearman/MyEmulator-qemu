...
Reset vectors and all future 16-bit values are little endian. QEMU initially
maps the full space as flat RAM, then overlays the floppy MMIO range. When a
floppy image is attached without `-kernel`, the built-in bootstrap ROM is
overlaid at `0x0180` and the reset PC vector points there. The bootstrap copies
sector 0 to `0x0100` before branching to it.
