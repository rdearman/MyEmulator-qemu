...
immediate `ADD`, register-to-register `CMP`, the six defined `0x6` conditional
branches, `JAL`, mask-based `PUSH`/`POP`, and `0xf080 HALT` are the current
defined bring-up subset. Illegal encodings stop visibly.
