...
| Opcode `0x7` | ARCHITECTURAL | Address/control/status family containing `LDA`, `GTA`, `MVA`, `ADA`, `JA`, `JLA`, `GF`, and `SF`; unused subencodings remain reserved. |
| BL | ARCHITECTURAL | Primary opcode `0x5`; writes `LR=P+2` and branches with the same signed PC-relative displacement. |
| Address registers | ARCHITECTURAL | `A0`-`A3` are 16-bit general address registers. `LD`/`ST` use `A[n] + sign_extend(disp8)`. |
| Address operations | ARCHITECTURAL | Primary opcode `0x7` contains `LDA`, `GTA`, `MVA`, `ADA`, `JA`, `JLA`, `GF`, and `SF` in disjoint subgroups; they do not modify S0 except SF. Revised MVA selectors cover A0-A3, LR, and SP, never PC. |
| Instruction alignment | ARCHITECTURAL | PC targets must be even. Odd JA/JLA/RET targets enter the synchronous handler at `0xffec`; the faulting PC and S0 use the existing three-byte RTI frame. |
...
expanded instruction groups require separate decisions. Plain `J`, `JA/JLA`,
